	...
	for (uint32_t check_event_index = 0; check_event_index < check_results->size(); ++check_event_index) {
		...
		for (uint32_t offset = start_offset; offset <= end_offset; ++offset) {
			...
			for (const auto& onset : op_group.onsets) {
				if (onset.qubit_index == qubit) {
					...
					onset_branches.push_back({op_index, p_erase, 0});
					p_unerased -= p_erase;
				}
			}
		...
		for (uint32_t offset = start_offset; offset <= end_offset; ++offset) {
			...
			while (branch_index < onset_branches.size() &&
				...
				p_erased_by_op += onset_branches[branch_index].probability;
				branch_index++;
			}
			...
			if (p_erased_by_op <= 0.0) {
				...
				for (const auto& spread : op_group.spreads) {
					...
					if (p_x > 0.0) {
						buckets[op_index].push_back(
							{op_index, spread.aff_qubit_index, simulator::PauliOperation::X, p_x});
					}
					if (p_y > 0.0) {
						buckets[op_index].push_back(
							{op_index, spread.aff_qubit_index, simulator::PauliOperation::Y, p_y});
					}
					if (p_z > 0.0) {
						buckets[op_index].push_back(
							{op_index, spread.aff_qubit_index, simulator::PauliOperation::Z, p_z});
					}
				}
			}
		}
...
stim::Circuit SurfHMMDecoder::decode(
	...
	for (uint32_t op_index = 0; op_index < program_.operation_groups.size(); ++op_index) {
		...
		for (const SpreadInjectionEvent& event : buckets[op_index]) {
			const double clamped_probability = std::clamp(event.probability, 0.0, 1.0);
			if (clamped_probability > 0.0) {
				switch (event.operation) {
					case simulator::PauliOperation::X:
						injected.safe_append_ua("X_ERROR", {event.target_qubit}, clamped_probability);
						break;
					case simulator::PauliOperation::Y:
						injected.safe_append_ua("Y_ERROR", {event.target_qubit}, clamped_probability);
						break;
					case simulator::PauliOperation::Z:
						injected.safe_append_ua("Z_ERROR", {event.target_qubit}, clamped_probability);
						break;
