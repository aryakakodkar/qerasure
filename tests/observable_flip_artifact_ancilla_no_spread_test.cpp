...
int main() {
	...
	constexpr uint32_t kSeedBase = 20260401;

	SurfaceCodeRotated generator(kDistance);
	const ErasureCircuit erasure_circuit =
		generator.build_circuit(kRounds, kErasureProb, /*erasable_qubits=*/"ANCILLA");
	...
	SurfDemBuilder dem_builder(compiled);

	const std::filesystem::path output_dir =
		std::filesystem::path("tests") / "artifacts" / "observable_flip_example_ancilla_no_spread";
	...
	for (uint32_t attempt = 0; attempt < kMaxTries; ++attempt) {
		...
		const stim::Circuit decoded_circuit =
			dem_builder.build_decoded_circuit(&check_bits, /*verbose=*/true);
	...
	std::cout << "observable_flip_artifact_ancilla_no_spread_test\n";
