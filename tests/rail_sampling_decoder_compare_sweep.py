#!/usr/bin/env python3
"""Compare decoders on the same rail-sampled data.

This sweep uses the rail sampling model for every point, then decodes the exact
same sampled shots with:
1. rail_calibrated (RailSurfaceDemBuilder + per-point calibration)
2. standard_dem (SurfDemBuilder pathway)
"""
...
def build_standard_dem_model(check_prob: float):
    model = qe.ErasureModel(
        2,
        qe.PauliChannel(0.25, 0.25, 0.25),
        qe.PauliChannel(0.25, 0.25, 0.25),
...
def sample_rail_data(
    *,
    ...
    return dets, obs, checks, {"sample": float(t1 - t0)}


def decode_rail_from_samples(
    *,
    rail_program: qe.RailSurfaceCompiledProgram,
    dets: np.ndarray,
    obs: np.ndarray,
...
def decode_standard_from_rail_samples(
    *,
        ...
        for p in p_values:
            ...
            for cfg_idx, (distance, rounds) in enumerate(configs):
                for scheme, marker, linestyle in (
                    ("standard_dem_on_rail_samples", "o", "-"),
                    ("rail_calibrated", "s", "--"),
                ):
                    curve = [
                        row
            ...
            ax.set_title(f"Decoder Comparison on Rail Samples (q={q:.6g}, p={p:.6g})")
            # Draw major and minor x-grid lines so log-spaced e values are easier to read.
            ax.xaxis.set_minor_locator(LogLocator(base=10.0, subs=np.arange(2, 10) * 0.1))
            ax.grid(True, which="major", alpha=0.35)
            ax.grid(True, which="minor", axis="x", alpha=0.22)
            ax.legend(fontsize=8)
            fig.tight_layout()
            out_path = plot_dir / f"rail_sample_decoder_compare_q{q:.6g}_p{p:.6g}.png"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Compare rail-calibrated decoder vs standard DEM decoder on the same rail-sampled shots. "
            "Runs non-final and final-round calibration before each (distance, rounds, e, p, q) point."
        )
    )
    ...
    parser.add_argument(
        "--json-out",
        type=Path,
        default=REPO_ROOT / "tests" / "artifacts" / "rail_sampling_decoder_compare_sweep.json",
    )
    parser.add_argument(
        "--plot-dir",
        type=Path,
        ...
        for p_idx, p in enumerate(p_values):
            for cfg_idx, (distance, rounds) in enumerate(configs):
                for e_idx, e in enumerate(e_values):
                    ...
                    model = build_model(check_prob=float(q))
                    rail_program = qe.RailSurfaceCompiledProgram(
                        circuit=circuit,
                        model=model,
                        distance=int(distance),
                    ...
                    standard_dem_model = build_standard_dem_model(check_prob=float(q))
                    compiled = qe.CompiledErasureProgram(circuit, standard_dem_model)

                    dets, obs, checks, sample_timing = sample_rail_data(
                        rail_program=rail_program,
                        shots=int(args.shots),
                        seed=int(point_seed),
                    )

                    rail_row = decode_rail_from_samples(
                        rail_program=rail_program,
                        dets=dets,
                        obs=obs,
                        checks=checks,
                    ...
                    rail_row["timing_seconds"]["sample"] = float(sample_timing["sample"])
                    rail_row["timing_seconds"]["total"] += float(sample_timing["sample"])
                    rail_row.update(
                        {
                    ...
                    standard_row = decode_standard_from_rail_samples(
                        compiled=compiled,
                        dets=dets,
                        obs=obs,
                        checks=checks,
                    ...
                    standard_row["timing_seconds"]["sample"] = float(sample_timing["sample"])
                    standard_row["timing_seconds"]["total"] += float(sample_timing["sample"])
                    standard_row.update(
                        {
                            "scheme": "standard_dem_on_rail_samples",
                            "sampling_model": "rail",
                            "distance": int(distance),
                            "rounds": int(rounds),
                            "erasure_prob": float(e),
                    ...
                    print(
                        ...
                        f"standard-on-rail LER/round={standard_row['logical_error_rate_per_round']:.6g}",
                        flush=True,
                    )
